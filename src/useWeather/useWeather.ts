    ...
    try {
      ...
      if (geolocationResponse.data && geolocationResponse.data.location) {
        ...
        if (weatherResponse.status === 200) {
          ...
          const minimumTemperature = Math.round(weatherData.main.temp_min);
          const maximumTemperature = Math.round(weatherData.main.temp_max);

          return { currentTemperature, description, minimumTemperature, maximumTemperature };
        } else {
